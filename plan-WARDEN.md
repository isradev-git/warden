...
| **Fase 4** *(opcional, lo recortado)* | **CVE de paquetes (OSV.dev)** ✅ — `cve` enumera paquetes (dpkg/rpm/pacman) y los consulta en batch contra api.osv.dev. **Histórico/tendencias** ✅ — `record` añade un snapshot (JSONL en `XDG_DATA_HOME`), `history` muestra sparklines de score y vitales; el dashboard auto-registra. **Descartado por decisión de alcance (2026-06-19):** binario `pyinstaller`, TUI Textual y soporte real de 2.º SO — WARDEN_ se queda **solo terminal + `rich`**. *Posible si algún día hace falta:* ejecución/programación de scripts, config TOML. Proyecto completo para su alcance. |
